...
{
  const options = [
    {label: "By Day", value: (a, b) => a.name.localeCompare(b.name)},
    {label: "Calories - Low to High", value: (a, b) => a.value - b.value},
    {label: "Calories - High to Low", value: (a, b) => b.value - a.value}
  ];
...
d3.csvParse(await FileAttachment("caloriesCounted.csv").text(), ({day, calories}) => ({name: day, value: +calories}))
)});
